...
namespace HutongGames.PlayMaker.Actions
{
    ...
    public class ForieroPlayerPrefsGetInt : FsmStateAction
    {
                ...
                if (!keys[i].IsNone || !keys[i].Value.Equals(""))
                {
                    switch (scopeEnum)
                    {
                        case ScopeEnum.PlayePrefs:
                            variables[i].Value = PlayerPrefs.GetInt(keys[i].Value, variables[i].IsNone ? 0 : variables[i].Value);
                            break;
                        case ScopeEnum.Player:
                            variables[i].Value = PlayerManager.player.GetInt(keys[i].Value, variables[i].IsNone ? 0 : variables[i].Value);
                            break;
                    }
                }
