...
namespace HutongGames.PlayMaker.Actions
{
	...
	[Tooltip("Follows an object by looking at it.")]
	public class EasingLookUpdate : FsmStateAction
	{
		...
		public override void Reset (){
			...
			followCursor = new FsmBool{UseVariable = true};
			go = null;
		}
		...
		public override void OnEnter ()
		{
			...
			if(go){
				Fsm.Owner.StartCoroutine(go.transform.LookUpdate(duration.IsNone ? 0f : duration.Value,
				                                                 delay.IsNone ? 0f : delay.Value,
				                                                 smoothTime.IsNone ? 1f : smoothTime.Value,
				                                                 target.IsNone ? null : (target.Value == null ? null : target.Value.transform),
				                                                 offset.IsNone ? Vector3.zero : offset.Value,
				                                                 followCursor.IsNone ? false : followCursor.Value,
