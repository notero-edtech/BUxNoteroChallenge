...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class EasingShake : FsmStateAction
	{
		public enum ShakeEnum{
			Position,
			Rotation,
			Scale
		}
		...
		[Tooltip("Shake Type.")]
		public ShakeEnum shakeType;
		public Space space;
		[RequiredField]
		[Tooltip("Vector3 shake.")]
		public FsmVector3 shakeVector;
		...
		public override void Reset (){
			...
			category = "Shake";
			shakeVector = new FsmVector3{UseVariable = true};
			duration  = new FsmFloat{ Value = 1f};
			delay = new FsmFloat{UseVariable = true};
			startedEvent = null;
			finishedEvent = null;
			shakeType = ShakeEnum.Position;
		...
		public override void OnEnter ()
		{
			...
			if(go){
				switch(shakeType){
				case ShakeEnum.Position :
					Fsm.Owner.StartCoroutine(go.transform.ShakePosition(
																	...
																	id_param));
				break;
				case ShakeEnum.Rotation :
					Fsm.Owner.StartCoroutine(go.transform.ShakeRotation(
						                                            duration.IsNone ? 1f : duration.Value,
						                                            delay.IsNone ? 0f : delay.Value,
				                                                    shakeVector.Value,
					                                                space,
																	...
																	id_param));
				break;
				case ShakeEnum.Scale :
					Fsm.Owner.StartCoroutine(go.transform.ShakeScale(
						                                            duration.IsNone ? 1f : duration.Value,
						                                            delay.IsNone ? 0f : delay.Value,
				                                                    shakeVector.Value,
				                                                    () => { if(startedEvent != null) Fsm.Event(startedEvent);},
