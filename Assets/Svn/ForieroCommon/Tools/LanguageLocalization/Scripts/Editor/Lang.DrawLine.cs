...
namespace UnityEditor{
	public static class Drawing
	{
		...
		public static void DrawNodeCurve(Vector2 start, Vector2 end, Color color, float width, float angle, bool shadow) {
			Vector3 startPos = new Vector3(start.x, start.y);
			Vector3 endPos = new Vector3(end.x, end.y);
			Vector3 startTan = startPos + Vector3.up * angle;
			Vector3 endTan = endPos + Vector3.down * angle;
			if(shadow){
