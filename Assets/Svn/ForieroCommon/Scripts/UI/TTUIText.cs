...
public class TTUIText : MonoBehaviour
{
    ...
    public static List<TTUIText> labels = new List<TTUIText>();

    public static void RefreshAll()
    {
        foreach (TTUIText label in labels)
        {
            label.Refresh();
        }
    }
    ...
    public void Refresh()
    {
        if (fontDefinition)
        {
            text.font = fontDefinition.GetFont(Lang.selectedLanguage);
            if (fontScaleFactor)
                text.fontSize = Mathf.FloorToInt(fontDefinition.GetScaleFactor(Lang.selectedLanguage) * originalFontSize);
        }
    ...
    void ResizeSelf()
    {
        ...
        if (text.horizontalOverflow == HorizontalWrapMode.Wrap)
        {
            this.FireAction(1, () =>
            {
                if (parentHeight < (text.preferredHeight * text.cachedTextGenerator.lineCount))
                {
                    scaleFactor = parentHeight / (text.preferredHeight * text.cachedTextGenerator.lineCount);
                    scaleFactor *= 1.25f;
                }
                if (fontScaleFactor)
                {
                    text.fontSize = Mathf.FloorToInt(fontDefinition.GetScaleFactor(Lang.selectedLanguage) * originalFontSize * scaleFactor);
                }
                else
                {
