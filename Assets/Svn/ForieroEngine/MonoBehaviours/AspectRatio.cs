...
public class AspectRatio : MonoBehaviour
{
	...
	void Apply ()
	{
		if (continuous) {
			if (onResolutionChange) {
                if (!Mathf.Approximately(Screen.width,resolution.x) || !Mathf.Approximately(Screen.height, resolution.y)) {
					resolution = new Vector2 (Screen.width, Screen.height);
					AdjustCameraViewPort ();
				} 
			} else {
	...
	void AdjustCameraViewPort ()
	{
        ...
        if (!Mathf.Approximately(finalResult, cam.orthographicSize)) {
