...
public class MIDIFrameReceiver extends MidiReceiver {
    ...
    public MIDIFrameReceiver(MidiReceiver receiver) { mReceiver = receiver; }
    ...
    public void onSend(byte[] data, int offset, int count, long timestamp) throws IOException {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (currentInt >= 0x80) { // status byte?
                ...
                if (!mInSysEx) {
                    ...
                    if (--mNeeded == 0) {
                        if (mRunningStatus != 0) { mBuffer[0] = mRunningStatus; }
        ...
        if (sysExStartOffset >= 0 && sysExStartOffset < offset) { mReceiver.send(data, sysExStartOffset, offset - sysExStartOffset, timestamp); }
    }
}
