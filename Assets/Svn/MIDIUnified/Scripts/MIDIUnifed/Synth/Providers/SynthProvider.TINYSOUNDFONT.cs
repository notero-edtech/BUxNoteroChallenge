...
namespace ForieroEngine.MIDIUnified.Synthesizer
{
    ...
    {
        class TINYSOUNDFONDSynthProvider : SynthProvider, ISynthRecorder
        {
            ...
            {
               if(CSharpSynth.singleton)  TinySoundFontSynth.singleton.StartRecording(bgClip, volume);
            }

            public void StopRecording()
            {
                if (CSharpSynth.singleton) TinySoundFontSynth.singleton.StopRecording();
            }
            ...
            static class Plugin
            {
                ...
                {
                    TinySoundFontSynth.StartSynthesizer(settings);
                    return 1;
                }

                public static int Stop()
                {
                    TinySoundFontSynth.StopSynthesizer();
                    return 1;
                }

                public static int SendShortMessage(int Command, int Data1, int Data2)
                {
                    TinySoundFontSynth.SendShortMessage((byte)Command, (byte)Data1, (byte)Data2);
                    return 1;                    
                }
            }
#else
