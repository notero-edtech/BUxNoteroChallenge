...
public class MidiEventsTemplate : MonoBehaviour
{
	...
	{
		if (hookMidiOut)
			midiEvents.AddSender (midiOutMonitor);
		if (hookSequencer)
			midiEvents.AddSender (midiSequencerScript);
		if (hookInput)
		...
		midiEvents.ShortMessageEvent += RawMidi;
	...
	void RawMidi (int aCommand, int aData1, int aData2, int deviceId)
	{
		if (rawMidi) {
			Debug.Log (string.Format ("MU | Command : {0} Data1 : {1} Data2 : {2}", aCommand, aData1, aData2));
			// how to get channel? //
			// how to get command? //
	...
	{
		if (noteOn)	Debug.Log (string.Format ("MU | Note ON : {0} Volume : {1} Channel : {2}", aNote, aVolume, aChannel));	
	}

	void NoteOff (int aNote, int aVolume, int aChannel)
	{
		if (noteOff) Debug.Log (string.Format ("MU | Note OFF : {0}  Channel : {1}", aNote, aChannel));	
	}

	void PedalOn (PedalEnum aPedal, int aValue, int aChannel)
	{
		if (pedalOn) Debug.Log ("MU | Pedal ON : " + aPedal.ToString () + " Channel : " + aChannel);
	}

	void PedalOff (PedalEnum aPedal, int aValue, int aChannel)
	{
		if (pedalOff) Debug.Log ("MU | Pedal ON : " + aPedal.ToString () + " Channel : " + aChannel);
	}
