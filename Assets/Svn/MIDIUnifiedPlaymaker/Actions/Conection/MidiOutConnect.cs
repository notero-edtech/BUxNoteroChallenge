...
namespace HutongGames.PlayMaker.Actions
{
	...
	[Tooltip ("Connect to a MIDI OUT device(port).")]
	public class MidiOutConnect : FsmStateAction
	{
		[Tooltip ("Midi Out Device(Port).")]
		public FsmInt Port;
		[Tooltip ("If you specify a PortName the action will try to find a Port with such a string")]
		public FsmString PortName;
		[Tooltip ("Called when a Midi Out port is successfuly connected.")]
		public FsmEvent connectedEvent;
		[Tooltip ("Called when a Midi Out port is not successfuly connected.")]
		...
		public override void OnEnter ()
		{
			if (!PortName.IsNone) {
				for (int i = 0; i < MidiOUTPlugin.GetDeviceCount (); i++) {
					string name = MidiOUTPlugin.GetDeviceName (i);
		...
		{
			MidiDevice device = MidiOUTPlugin.ConnectDevice (aPort);
