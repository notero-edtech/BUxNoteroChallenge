...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class MidiGeneratorShortMessageListener : FsmStateAction
	{
		[RequiredField]
		public FsmObject generator;
		...
		public override void OnEnter ()
		{
			...
			_midiSender = generator.Value as IMidiSender;

			if (_midiSender != null) {
				_midiSender.ShortMessageEvent += shortMessageDelegate;
			} else {
				Debug.LogError (generator.Value.name + " does not contains IMidiEvent;");
			}
		}
		...
		void OnShortMessage (int aCommand, int aData1, int aData2, int deviceId)
		{
			bool result = true;
			if (!channel.IsNone) {
				if (channel.Value != (aCommand.ToMidiChannel ()))
					result = false;
			}
			
			if (!command.IsNone) {
				if (command.Value != aCommand.ToMidiCommand ())
					result = false;
				else {
					if (!data1.IsNone) {
						if (data1.Value != aData1)
							result = false;
						else {
							if (!data2.IsNone) {
								if (data2.Value != aData2)
									result = false;
							}
						}
					}
			...
			if (result) {
				if (!channelData.IsNone)
					channelData.Value = aCommand.ToMidiChannel ();
				if (!commandData.IsNone)
					commandData.Value = aCommand.ToMidiCommand ();
				if (!data1Data.IsNone)
					data1Data.Value = aData1;
				if (!data1Data.IsNone)
					data2Data.Value = aData2;
