                        ...
                        internal static partial class NoteRest
                        {
                            internal static partial class Note
                            {
                                ...
                                internal static void Parse(note note)
                                {
                                    ...
                                    nsNote = nsStaveFixed.AddObject<NSNote>(PoolEnum.NS_FIXED);
                                    ...
                                    nsDurationBar = nsStaveFixed.AddObject<NSDurationBarHorizontal>(PoolEnum.NS_FIXED);
                                    nsDurationBar.passable = true;
                                    nsDurationBar.options.lenght = timePixelsInMeasure * directionSign;
                                    ...
                                    if (tieType == TieTypeEnum.Stop || tieType == TieTypeEnum.Undefined) nsMidiEventEnd = nsPart.AddObject<NSObject>(PoolEnum.NS_FIXED); else nsMidiEventEnd = null;
                                    ...
                                    if (spNote.IsChord())
                                    {
                                        ...
                                        if (nsMidiEventEnd) nsMidiEventEnd.PixelShiftX((fbPreviousPixels + timePixelsInMeasure - shortenDurationBarsForPixels) * directionSign, true);
                                        if (nsMidiEventEnd) nsMidiEventEnd.pixelTime = fbPreviousPixels + timePixelsInMeasure - shortenDurationBarsForPixels;
                                    }
                                    else
                                    {
                                        ...
                                        if (nsMidiEventEnd) nsMidiEventEnd.PixelShiftX((fbPixels + timePixelsInMeasure - shortenDurationBarsForPixels) * directionSign, true);
                                        if (nsMidiEventEnd) nsMidiEventEnd.pixelTime = fbPixels + timePixelsInMeasure - shortenDurationBarsForPixels;
                                    }
                                    ...
                                    if (NSDisplaySettings.Ties)
                                    {
                                        switch (tieType)
                                        {
                                            case TieTypeEnum.Start:
                                                nsNote.tie = nsNote.AddObject<NSTie>(PoolEnum.NS_FIXED,
