...
namespace ForieroEngine.Music.NotationSystem.Systems
{
    public partial class NSRollingTickerSystem : NS
    {
        ...
        {
			var directionFix = NSPlayback.NSTickerPlayback.rollingMode == NSPlayback.NSTickerPlayback.RollingMode.Left ? -1f : 1f;
            ...
            foreach (NSPart part in parts)
            {
				...
				foreach (NSStave stave in staves)
                {
					...
					stave.Arrange(NSPlayback.NSTickerPlayback.rollingMode.ToHorizontalDirectionEnum());
				}
            }
        }
    }
