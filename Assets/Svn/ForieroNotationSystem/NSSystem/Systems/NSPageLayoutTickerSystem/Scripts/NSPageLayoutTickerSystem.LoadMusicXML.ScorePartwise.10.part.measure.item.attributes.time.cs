                                ...
                                internal static void Parse(time[] times, PoolEnum poolEnum)
                                {
                                    ...
                                    for (int timeIndexer = 0; timeIndexer < spTimes.Length; timeIndexer++)
                                    {
                                        ...
                                        for (int i = minStave; i <= maxStave; i++)
                                        {
                                            ...
                                            NSTimeSignature._options.changing = measureIndexer > 0;
                                            NSTimeSignature timeSignature = nsPart.parsing.staves[i].AddTimeSignature(NSTimeSignature._options, PivotEnum.MiddleCenter, poolEnum);

                                            nsPart.parsing.staves[i].parsing.timeSignature.CopyValuesFrom(NSTimeSignature._options);
                                            ...
                                            switch (poolEnum)
                                            {
                                                case PoolEnum.NS_MOVABLE:
                                                    {
                                                        ...
                                                        timeSignature.pixelTime = Part.fbPixels - ns.LineSize * 1.6f;
                                                        timeSignature.PixelShiftX(timeSignature.pixelTime, true);
                                                        if (measureIndexer == 0)
                                                        {
                                                ...
                                                case PoolEnum.NS_FIXED:
                                                    {
                                                        timeSignature.passable = false;
                                                        nsPart.parsing.staves[i].timeSignature = timeSignature;
