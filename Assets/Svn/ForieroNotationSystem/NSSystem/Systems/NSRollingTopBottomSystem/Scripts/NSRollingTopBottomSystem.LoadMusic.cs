    ...
    public partial class NSRollingTopBottomSystem : NS
    {
        ...
        public override void LoadMusic(float[] samples, int channels, float totalTime)
        {
            ...
            if (NS.debug)
            {
                ...
                Debug.Log("NS PixelsPerSecond : " + NSPlayback.NSRollingPlayback.pixelsPerSecond.ToString());
                Debug.Log("AUDIO Time : " + totalTime.ToString());
                Debug.Log("TRACK PixelLenght : " + (NSPlayback.NSRollingPlayback.pixelsPerSecond * totalTime).ToString());
            }
