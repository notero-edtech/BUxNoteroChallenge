...
namespace ForieroEngine.Music.Detection.Pitch
{
    public static class Freq2Note
    {
        ...
        static int DominantFreqIndex(float AnalyzedFreq)
        {
            int result = 0;
            double diff = 9999999.0;
            for (int i = 0; i < notefreq.Length; i++)
            {
                if (System.Math.Abs(AnalyzedFreq - notefreq[i]) < diff)
                {
                    result = i;
                    diff = System.Math.Abs(AnalyzedFreq - notefreq[i]);
                }
                else
                {
                    break;
        ...
        static public string DominantNote(float AnalyzedFreq)
        {
            return note[DominantFreqIndex(AnalyzedFreq)];
        }

        static public float DominantFreq(float AnalyzedFreq)
        {
            return notefreq[DominantFreqIndex(AnalyzedFreq)];
        }
