...
namespace ForieroEngine.Music.Detection.Pitch
{
    ...
    public class Tuner
    {
        ...
        internal float FindFundamentalFrequency(float[] x, int sampleRate, float minFreq, float maxFreq)
        {
            float[] spectr = SoundAnalysis.FftAlgorithm.Calculate(x);
        ...
        private void ScanSignalIntervals(float[] x, int index, int length,
