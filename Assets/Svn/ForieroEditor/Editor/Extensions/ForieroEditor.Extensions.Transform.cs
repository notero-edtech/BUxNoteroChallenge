...
namespace ForieroEditor.Extensions
{
    ...
    {
        public static void DoRecursive(this GameObject o, Action<GameObject> doAction, bool recursive)
        {
            doAction?.Invoke(o);
            if (!recursive) return;
            foreach (Transform t in o.transform) DoRecursive(t.gameObject, doAction, true);
        }

        public static GameObject[] GetUndoGameObjects(this GameObject[] gameObjects, bool recursive)
        {
            var l = new List<GameObject>();
            l.AddRange(gameObjects);
            if (!recursive) return l.ToArray();
            foreach (var go in gameObjects) l.AddRange(go.GetUndoGameObjects(true));            
            return l.ToArray();
        }
        
        public static GameObject[] GetUndoGameObjects(this GameObject gameObject, bool recursive)
        {
            var l = new List<GameObject>();
            l.Add(gameObject);
            if (!recursive) return l.ToArray();
            l.AddRange(gameObject.transform.GetComponentsInChildren<Transform>().Select((t) => t.gameObject));
            return l.ToArray();
        }
